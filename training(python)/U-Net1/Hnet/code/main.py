...
def readdata(index, path):
    ...
    input[1, 0, :, :] = img[:]
    # 读取输出
    img_add = path + 'gt/M(' + str(index[0]) + ').mat'
    mat_train = h5py.File(img_add,'r')
    img = mat_train['M'][()].T
    output[0, 0, :, :] = img[:]
    img_add = path + 'gt/D(' + str(index[0]) + ').mat'
    mat_train = h5py.File(img_add,'r')
    img = mat_train['D'][()].T
    output[0, 1, :, :] = img[:]

    img_add = path + 'gt/M(' + str(index[1]) + ').mat'
    mat_train = h5py.File(img_add,'r')
    img = mat_train['M'][()].T
    output[1, 0, :, :] = img[:]
    img_add = path + 'gt/D(' + str(index[1]) + ').mat'
    mat_train = h5py.File(img_add,'r')
    img = mat_train['D'][()].T
    output[1, 1, :, :] = img[:]
    # 转tensor
...
if __name__ == '__main__':
    ...
    t.train(300, r"../dataset-MD/train/", r"../dataset-MD/valid/")
